...
class TestPKCEGeneration(unittest.TestCase):
    ...
    def test_challenge_is_sha256_of_verifier(self):
        ...
        digest = hashlib.sha256(
            self.pkce.code_verifier.encode('ascii')
        ).digest()
...
class TestBuildAuthUrl(unittest.TestCase):
    ...
    def setUp(self):
        from hamster.auth.oauth import generate_pkce_params, generate_state, build_auth_url
        self.pkce    = generate_pkce_params()
...
class TestOAuthCallbackHandler(unittest.TestCase):
    ...
    def test_successful_callback_returns_code_and_state(self):
        ...
        with patch.object(
            SingleRequestHTTPServer, '__init__',
            wraps=lambda self_inner, es, p=port: SingleRequestHTTPServer.__bases__[0].__init__(
                self_inner, ('localhost', port), __import__('hamster.auth.server', fromlist=['OAuthCallbackHandler']).OAuthCallbackHandler
            ) or setattr(self_inner, 'expected_state', es)
                  or setattr(self_inner, 'callback_result', None)
                  or setattr(self_inner, 'callback_received', __import__('threading').Event()),
        ):
...
class TestSecureTokenStoreFileFallback(unittest.TestCase):
    ...
    def test_file_save_and_load_roundtrip(self):
        payload = {"email": "test@example.com", "access_token": "tok123", "sub": "1234567890"}
...
class TestGetProfile(unittest.TestCase):
    ...
    def test_profile_extracted_from_id_token(self):
        ...
        claims   = {"sub": "uid-001", "email": "alice@example.com", "name": "Alice", "picture": "https://img"}
        jwt      = self._make_jwt(claims)
