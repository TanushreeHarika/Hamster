...
class SecretRedactor:
    DEFAULT_PATTERNS = [re.compile(r"AKIA[0-9A-Z]{16}"), re.compile(r"[A-Za-z0-9_\-]{32,}")]
...
class AuditLogger:
    ...
    def log_event(self, session_id: str, event_type: str, payload: Dict[str, Any]) -> None:
        event = AuditEvent(timestamp=datetime.utcnow().isoformat() + "Z", session_id=session_id, event_type=event_type, payload=payload)
